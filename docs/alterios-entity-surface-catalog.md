...
Дополнительные правила по связям:

- `ref source=view` настраивается через `views`, `entityContentType`,
  `entityType=content`, `widget=autocomplete` и `valueCount`;
- такое поле хранит id связанной content-записи; само по себе оно не гарантирует
  человекочитаемый вывод в списке;
- если пользователю нужно видеть атрибут связанной записи, например ФИО
  сотрудника, надо добавить reference/source view или joined `table` view;
- `person` хранит структурированный объект ФИО, но не заменяет `ref` на
  сотрудника/пользователя и требует отдельной проверки отображения в форме или
  таблице.
...
Для UI-preview `settings.title` у experimental table/reference/list задается как
один фактический `viewField.mname`. Шаблон с `{{...}}` может пройти API-save, но
дать ошибку предпросмотра в пользовательском интерфейсе.
...
Позднее live-исследование связи подтвердило другой UI-случай: для штатного
`Сохранить и запустить предпросмотр` в форме редактирования view пустой
`settings.title` блокирует preview, а корректное значение - это mname одного
view field. Поэтому typed tool может не требовать `settings.title` для API
smoke, но пользовательский UI-preview должен проверять и заполнять его.
