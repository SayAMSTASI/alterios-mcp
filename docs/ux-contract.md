...
## Единая проверка модуля

`alterios_validate_module_contract` проверяет одним вызовом тип материала и
подсказки полей, experimental/v2 view, entities/joins/view fields, комплект
add/edit/view/list форм, действия и иконки, optional bulk-интерфейс, отчеты и
их Project Database source. Offline-режим принимает готовый `module` JSON и не
обращается к Alterios. Live-режим принимает точные ID и выполняет 9 базовых
API-вызовов плюс один вызов на каждый явно переданный отчет.

Проверка не запускает project inventory, не сканирует файловый менеджер и по
умолчанию не скачивает SVG. Для иконок сначала используются локальный registry,
SHA-256 и git-библиотека. `verify_icon_files=true` скачивает только неразрешенные
иконки, не более `max_icon_files` (по умолчанию 16, абсолютный предел 32).

Дополнительные блокирующие правила модуля:

- тип материала имеет содержательное пользовательское описание;
- каждое поле имеет tooltip/help;
- view использует `settings.engineVersion=v2`, содержит entity и поля;
- дочерние view entities имеют корректные joins;
- присутствуют add/edit/view/list формы;
- действия элементов view/edit совпадают, кроме перехода view -> edit;
- обязательный bulk-интерфейс присутствует, если передан `require_bulk_interface=true`;
- `iconId` разрешается через project-local registry, семантика соответствует
  действию, source size равен 16, SVG canvas равен 20px, цвет `#4B77D1`;
- отчет содержит Project Database source, а source view имеет поля.

Низкоуровневый `alterios_upsert_content_type` показывает нарушение в dry-run и
блокирует apply, если итоговое описание отсутствует или содержит только
служебный marker.
