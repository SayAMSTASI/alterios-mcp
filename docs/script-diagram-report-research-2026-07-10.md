...
## 2026-07-12: UI/API evidence for all script types

The script create UI exposes six script types:

| UI label | Stored `type` | Notes |
|---|---|---|
| `Веб` | `web` | Save route accepts the type. Keep inactive until external endpoint behavior is verified. |
| `Крон` | `cron` | Save route accepts the type. `config.cron` is a six-part string: `second minute hour day month week`, for example `0 0 3 * * *`. |
| `Ручной` | `manual` | Executed by saved script UUID through `POST /api/scripts/execute-manual`. |
| `Событийный` | `event` | Stored as a script; trigger binding must be verified through event-side evidence. |
| `Библиотека` | `library` | Stored as a script and consumed by other scripts through `librariesIds`. Prefer global helper functions/constants unless runtime evidence proves another module format. |
| `Диаграмма` | `diagram` | Used by BPMN/script-task flow; verify through diagram refs and process/task smoke. |

Sandbox write evidence confirmed that `web`, `cron`, and `library` can be saved by `POST /api/scripts`.
For cron, the UI displays separate `second`, `minute`, `hour`, `day`, `month`, and `week` inputs, but the API stores them as one `config.cron` string.
Manual script execution returns HTTP 201 with an empty body in the tested endpoint, so success proof requires script readback plus a separate log/status/readback route when side effects matter.
