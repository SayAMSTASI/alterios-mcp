...
| PM Control Loop | В начале этапа и после каждого проверенного среза | Разбить цель на этапы, зафиксировать acceptance criteria, вести статус, риски, блокеры, следующий шаг | Приватная Gitea/local workboard, `docs/project-status.md`, backlog, текущая цель пользователя | Обновленный статус, stage gate, список следующих задач | Только private workboard/docs |
