...
Текущая версия: **0.2.1**. Публичный registry содержит **107 MCP tools**:
`live` - 80, `discovery` - 54, `admin` - 105, `full` - 107.
...
После обновления остановите старый MCP-процесс и перезапустите MCP-клиент.
Запущенный процесс не перечитывает новый код и registry автоматически.
...
|---|---:|---|
| `live` | 80 | Основной профиль: health, scenarios, typed writes и проверка результата |
| `discovery` | 54 | Read-only исследование, inventory и validators |
| `admin` | 105 | Администрирование, security и controlled destructive operations |
| `full` | 107 | Разработка MCP и исследование неизвестных routes |
...
3. Переустановите editable package.
