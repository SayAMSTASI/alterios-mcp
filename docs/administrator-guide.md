...
Рекомендуемая установка готового release wheel:
...
Для установленного release wheel скачайте `manage_release.ps1` из GitHub
Release. Скрипт принимает URL или локальный путь к проверенному wheel, запускает
doctor и release smoke и не завершает процессы самостоятельно.
...
Rollback выполняется тем же скриптом с сохраненным wheel предыдущей версии:

```powershell
.\manage_release.ps1 -Action Rollback `
  -Package "C:\path\to\previous\alterios_mcp-previous-py3-none-any.whl" `
  -DotenvPath "C:\path\to\private\alterios.env"
```
