...
В ART X sandbox на момент инвентаризации roles list был доступен, но roles count
равен `0`. Поэтому правила payload-а и UI-семантика ролей не считаются
подтвержденными.
...
До этого нельзя обещать native publish как реализованную MCP-возможность.

## 16. Что добавить в MCP следующим этапом

Приоритетные сценарии расширения:

1. `docs/security-destructive-evidence.md`:
   - users;
   - user groups;
   - roles;
   - permissions;
